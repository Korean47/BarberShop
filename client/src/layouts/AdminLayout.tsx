    ...
    <>
      <div className="flex h-[68px] items-center border-b border-white/10 px-3">
        <Link to="/" className="flex items-center gap-3" onClick={() => mobile && setMobileOpen(false)}>
          ...
          {(!collapsed || mobile) && <div><p className="text-base font-black text-white">Blades</p><p className="text-xs font-bold uppercase tracking-wider text-white/55">Administración</p></div>}
        </Link>
      </div>
      ...
      <div className="border-t border-white/10 p-3">
        {(!collapsed || mobile) && <div className="mb-2 flex items-center gap-3 rounded-xl bg-white/10 p-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-[#f2c14e] text-xs font-black text-[#17313a]">{session?.user.name.split(' ').map((part) => part[0]).slice(0, 2).join('')}</div><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold text-white">{session?.user.name}</p><p className="text-xs text-white/55">Administrador</p></div><button type="button" onClick={() => void logout()} className="grid h-9 w-9 place-items-center rounded-lg text-white/65 hover:bg-white/10 hover:text-white" aria-label="Cerrar sesión"><LogOut className="h-4 w-4" /></button></div>}
    ...
    <div className="admin-shell flex min-h-screen bg-[#f4f7f7] text-slate-900">
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-30 flex h-[68px] items-center border-b border-slate-200 bg-white/95 px-4 backdrop-blur lg:px-7">
          ...
          <div className="min-w-0"><p className="text-xs font-black uppercase tracking-wider text-slate-500">Panel de la barbería</p><h1 className="truncate text-base font-black text-slate-900">{pageNames[location.pathname] || 'Administración'}</h1></div>
