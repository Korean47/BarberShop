  ...
  return (
    <fieldset>
      ...
      <div className="mt-5 space-y-6">
        {categories.map((category) => <section key={category} aria-labelledby={`category-${category}`}><h2 id={`category-${category}`} className="mb-3 text-xs font-semibold uppercase tracking-[.14em] text-[var(--muted)]">{category}</h2><div className="grid gap-3 sm:grid-cols-2">{services.filter((service) => service.category === category).map((service) => {
