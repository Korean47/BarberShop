  ...
  return (
    <div className="section-container py-10 sm:py-16">
      <div className="mx-auto max-w-4xl">
        <div className="text-center">{appointment.status === 'cancelled' ? <XCircle className="mx-auto h-14 w-14 text-[var(--error)]" /> : <CheckCircle2 className="mx-auto h-14 w-14 text-[var(--success)]" />}<p className="mt-5 eyebrow">Mi cita</p><h1 className="type-heading-1 mt-2">{appointment.status === 'cancelled' ? 'Cita cancelada' : pendingPayment ? 'Pago pendiente' : 'Detalles de la cita'}</h1><p className="mt-3 text-sm text-[var(--muted)]">Código <strong className="type-booking-code text-[var(--text)]">{appointment.publicCode}</strong></p></div>
