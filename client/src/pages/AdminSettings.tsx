...
import { TypographyPackagePicker } from '../components/admin/TypographyPackagePicker';
import { Button } from '../components/ui/Button';
import { PageSpinner } from '../components/ui/Spinner';
import { getAdminSettings, updateAdminSettings } from '../services/api';
import type { AdminSettingsData } from '../types';
import { normalizeTypographyPackage, type TypographyPackageId } from '../config/typography';
...
interface SettingsForm {
  ...
  branding: {
    ...
    backgroundColor: string; fontFamily: TypographyPackageId; publish: boolean;
  };
...
function makeForm(data: AdminSettingsData): SettingsForm {
  ...
  return {
    ...
    branding: {
      ...
      backgroundColor: branding?.backgroundColor || '#F5F2EB', fontFamily: normalizeTypographyPackage(branding?.fontFamily), publish: true,
    },
    booking: {
    ...
    <div className="space-y-7">
      ...
      <section className="grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)]">
        ...
        <article className="admin-card min-w-0 p-5 lg:p-7">
          {activeTab === 'landing' && <TypographyPackagePicker value={form.branding.fontFamily} onChange={(fontFamily) => { document.documentElement.dataset.fontPackage = fontFamily; setForm({ ...form, branding: { ...form.branding, fontFamily } }); }} />}
