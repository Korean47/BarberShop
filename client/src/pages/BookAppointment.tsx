...
export function BookAppointment() {
  ...
  if (result) {
    ...
    return <div className="fixed inset-0 z-50 overflow-y-auto bg-[var(--background)]"><div className="section-container grid min-h-full place-items-center py-8"><div className="w-full max-w-2xl overflow-hidden rounded-[1.5rem] border border-[var(--stone)] bg-[var(--surface-light)] shadow-xl"><div className="bg-[#E8F3EC] p-7 text-center sm:p-9"><span className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-[var(--success)] text-white"><CheckCircle2 className="h-9 w-9" /></span><p className="mt-5 eyebrow text-[var(--success)]">Cita confirmada</p><h1 className="type-heading-1 mt-2">Tu cita quedó registrada</h1></div><div className="p-5 sm:p-8"><dl className="grid gap-4 rounded-2xl bg-[var(--background)] p-5 sm:grid-cols-2"><div><dt className="summary-label">Servicio</dt><dd className="summary-value">{appointment.service.name}</dd><p className="mt-1 text-sm text-[var(--muted)]">{appointment.barber.name}</p></div><div><dt className="summary-label">Fecha y hora</dt><dd className="summary-value capitalize">{formatDateLong(new Date(`${date}T12:00:00`))}</dd><p className="mt-1 text-sm text-[var(--muted)]">{formatTime(appointment.startTime)}</p></div><div><dt className="summary-label">Total</dt><dd className="type-price mt-1 text-[var(--accent)]">{formatPrice(appointment.totalCents / 100)} · pago en el local</dd></div><div><dt className="summary-label">Código de cita</dt><dd className="type-booking-code mt-1 text-xl">{appointment.publicCode}</dd></div></dl><a href={mapsUrl} target="_blank" rel="noreferrer" className="mt-4 flex min-h-12 items-center gap-2 rounded-xl border border-[var(--stone)] p-4 text-sm font-semibold"><MapPin className="h-5 w-5 shrink-0 text-[var(--accent)]" /> {address}</a><div className="mt-6 grid gap-3 sm:grid-cols-2"><a href={result.manageUrl} className="button-secondary">Consultar o modificar</a><button type="button" onClick={downloadCalendar} className="button-outline"><CalendarPlus className="h-5 w-5" /> Guardar en calendario</button></div><Link to="/" className="mt-5 block text-center text-sm font-semibold text-[var(--muted)]">Volver al inicio</Link></div></div></div></div>;
  }
